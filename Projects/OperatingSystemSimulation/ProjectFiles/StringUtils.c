...
Algorithm: find the length of the string
Precondition: given C-style string with null character at end
Postcondition: return the number of characters from beginning up to null
               character
Exceptions: none
...
int getStringLength( char *testStr )
{
    ...
    // look up to null character or limit
...
void copyString( char *destination, char *source )
{
    ...
    // loop until null character to destination element
...
Postcondtion: integer value returned as specified
Exceptions: none
Note: limit test loop to maximum characters for safety
*/
...
Precondition: given a C-style test string, having a null character ('\0')
              ...
              a null character ('\0') at the end of that string
Postcondition: index of substring location returned,
               or SUBSTRING_NOT_FOUND constant is required
Exceptions: none
Note: none
*/
...
Note: copy of source string is made internally to protect form aliasing
*/
...
Postcondition: ignores leading white space if specifed;

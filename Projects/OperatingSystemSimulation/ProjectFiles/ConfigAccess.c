...
Postcondition: in correct operation,
               returns pointer to correct confiruation data structure
    ...
    *configData = NULL;
    ...
    while( lineCtr < NUM_DATA_LINES )
    {
        ...
        if( dataLineCode != CFG_CORRUPT_PROMPT_ERR )
        {
            ...
            if( valueInRange( dataLineCode, intData, longData, doubleData, dataBuffer )
                                                                       == True )
            {
...
Algorithm: tests string for one of known leader strings, returns line number
           ...
           of string is not found
Precondition: dataBuffer is valid C-Style string
Postconidtion: returns line number of data item in terms of a constant
...
Notes: none
*/
Boolean valueInRange( int lineCode, int intVal, long longVal,
...
Postcondition: correct eror string (related to error code)
               is displayed
Exceptions: none
Notes: none
*/
